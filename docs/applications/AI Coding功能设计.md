...
AI Coding 是 GraphAgent 的核心应用层功能之一，旨在让 Agent 具备理解需求、设计方案、编写代码、验证测试的完整软件开发能力。该功能以 Skill 的形式实现，通过精心设计的 Prompt 引导学生（LLM）按照 **需求澄清 → 方案设计 → 用例设计 → 任务分解 → 代码开发 → 测试验证 → 返回结果** 的七步工作流完成开发任务。
...
└──────┬──────────┘
       ...
       ▼
┌─────────────────┐
│ 生成代码         │  ← 每个子任务代码量控制在 50-150 行
└──────┬──────────┘
       │
       ▼
┌─────────────────┐
│ 写入代码         │  ← 使用 update_code（修改已有文件）
│                 │     或 write_code（创建新文件）
│                 │     超过 200 行时分段写入
└──────┬──────────┘
       │
       ▼
┌─────────────────┐
│ 编写 UT 用例      │  ← 根据步骤三的用例设计编写具体的 UT 代码
└──────┬──────────┘
       │
       ▼
┌─────────────────┐
...
```
测试失败
    → 分析失败原因（断言错误 / 运行时异常 / 编译错误）
    → 定位问题代码（使用 grep_context 搜索相关位置）
    → 读取相关代码（使用 read_code 读取完整上下文）
    → 修改代码（使用 update_code 进行精准修改）
    → 重新执行测试（使用 execute_command 运行 pytest）
    → 仍失败 → 继续修改（最多重试 3 次）
    → 3 次仍失败 → 向用户报告当前状态和遇到的具体问题
```
...
| `glob` | string | 否 | 文件名过滤，如 `*.py` 仅搜索 Python 文件 |
