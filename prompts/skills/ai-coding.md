...
## 功能概述
作为 AI Coding 助手，遵循 TDD（测试驱动开发）原则，完成从需求分析到代码交付的完整软件开发流程。适用场景包括：新功能开发、Bug 修复、代码重构、项目脚手架搭建。
...
3. **编写测试（RED）**：根据阶段三的用例设计，使用 write_code 创建/追加 UT 用例
   → 确认 UT 此时运行会失败
4. **实现代码（GREEN）**：使用 update_code（修改已有文件）或 write_code（创建新文件）
